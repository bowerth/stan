...
## @param path.cou path to COU folder containing country folder.
                    ...
                    SDMX=FALSE)
{
    ...
    file = paste0(path.cou, cou, "\\Rawdata\\R_ANA_4SAS_2013-14.xls")
    ...
    path.mdlind <- paste0(path.cou, cou, '\\Lists\\MDL_IND_ANA.txt')
    if (SDMX==TRUE) path.mdlind <- sub("MDL_IND_ANA.txt", "MDL_IND_ANA_SDMX.txt", path.mdlind)
    path.mdlvar <- paste0(path.cou, cou, '\\Lists\\MDL_VAR_ANA.txt')
