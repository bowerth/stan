                     ...
                     export=TRUE)
{
    ...
    ##
    prog.batch <- file.path(path.sas, "progs", "PAYS", paste0("batchCou_", gsub(", ", "-", toString(prog.sas)), ".sas"))
    fileCon <- file(prog.batch)
    if (!file.exists(prog.batch)) file.create(prog.batch)
    prepend <- paste0("\n",
                      "/* %INCLUDE \"", file.path(path.sas, "progs", "0_Executable.sas"), "\" ; */\n",
                      "\n",
                      "%MACRO batchCou;\n",
                      "\n",
    ...
    if (export==TRUE) {
        append <- paste0("\tPROC EXPORT DATA=&cou..swn OUTFILE=\"", file.path(path.cou, "&cou.", "SASdata", "swn.csv"), "\" REPLACE; RUN ;\n", append)
    }
    ## cat(append)

    list.cou.string <- gsub(", ", " ", toString(namecou))
    lines.prog <- rbind.data.frame(c(TRUE, "LIST_COU", paste0("/* %INCLUDE \"", file.path(path.sas, "progs", "LISTS", "load STAN_cou.sas"), "\" ; */")),
                                   c(TRUE, "LIST_UNIT", paste0("/* %INCLUDE \"", file.path(path.sas, "progs", "LISTS", "load STAN_unit.sas"), "\" ; */\n")),
                                   ...
                                   c(TRUE, "LIBNAME_NEW", paste0("\tLIBNAME &cou \"", file.path(path.cou, "&cou.", "SASdata"), "\" ;\n")),
                                   c(FALSE, "LOAD", paste0("\t%INCLUDE \"", file.path(path.sas, "progs", "PAYS", "&cou.", "&cou._load.sas"), "\" ;\n")),
                                   c(FALSE, "ESTIM", paste0("\t%INCLUDE \"", file.path(path.sas, "progs", "PAYS", "&cou.", "&cou._estim.sas"), "\" ;\n")),
                                   c(FALSE, "PUB", paste0("\t%INCLUDE \"", file.path(path.sas, "progs", "PAYS", "&cou.", "&cou._pub.sas"), "\" ;\n")))
